...
class Episode:
    ...
    stream_counts: dict = field(default_factory=dict)
    last_timestamps: dict = field(default_factory=dict)
    qc_errors: list = field(default_factory=list)
...
class EpisodeWriter:
    ...
    def __init__(self, directory, episode):
        ...
        self._episodes_directory = self._directory / "episodes"
        self._base_directory = (
            self._episodes_directory / f".partial-{self._episode.number}"
        )
        self._final_directory = self._episodes_directory / str(self._episode.number)
        self._base_directory.mkdir(parents=True, exist_ok=False)
    ...
    def commit(self):
        """Atomically publish a fully written episode."""
        if self._final_directory.exists():
            raise ValueError(f"Episode directory already exists: {self._final_directory}")
        os.replace(self._base_directory, self._final_directory)

    def cancel(self):
        """Preserve a cancelled episode outside the trainable episode tree."""
        if not self._base_directory.exists():
            return None
        self._episode.status = "cancelled"
        self._episode.ended_at_ns = time.time_ns()
        self._write_episode_manifest()
        quarantine = self._directory / "quarantine"
        quarantine.mkdir(parents=True, exist_ok=True)
        destination = quarantine / (
            f"cancelled-{self._episode.number}-{self._episode.ended_at_ns}"
        )
        shutil.move(str(self._base_directory), str(destination))
        return destination

    def _write_episode_manifest(self):
        manifest = {
            "id": str(self._episode.number),
            "status": self._episode.status,
            "success": self._episode.success,
            "failure_reason": self._episode.failure_reason,
            "task_index": self._episode.task_index,
            "started_at_ns": self._episode.started_at_ns,
            "ended_at_ns": self._episode.ended_at_ns,
            "session": self._episode.session,
            "stream_counts": self._episode.stream_counts,
            "qc_errors": self._episode.qc_errors,
        }
        output_path = self._base_directory / "episode.yaml"
        with output_path.open("w", encoding="utf-8") as output:
            yaml.safe_dump(manifest, output, sort_keys=False, allow_unicode=True)
...
class DatasetWriter:
    ...
    def finish_episode(self, episode):
        ...
        result = {
            ...
            "stream_counts": episode.stream_counts,
            "qc_errors": episode.qc_errors,
        }
        self._episode_results = [
            existing
            for existing in self._episode_results
            if existing.get("id") != result["id"]
        ]
        self._episode_results.append(result)
        self._episode_results.sort(key=lambda item: int(item["id"]))
        self._write_metadata_file()
    ...
    def _quarantine_partial_episodes(self):
        ...
        if not episodes_directory.exists():
            return
        partials = sorted(episodes_directory.glob(".partial-*"))
        if not partials:
            return
        quarantine = self._base_directory / "quarantine"
        quarantine.mkdir(parents=True, exist_ok=True)
        recovered_at_ns = time.time_ns()
        for partial in partials:
            destination = quarantine / f"orphan-{partial.name.removeprefix('.partial-')}-{recovered_at_ns}"
            shutil.move(str(partial), str(destination))

    def _recover_committed_episodes(self):
        """Recover a commit that completed just before a metadata write crash."""
        episodes_directory = self._base_directory / "episodes"
        if not episodes_directory.exists():
            return
        known_ids = {str(result.get("id")) for result in self._episode_results}
        for episode_directory in sorted(episodes_directory.iterdir()):
            if not episode_directory.is_dir() or not episode_directory.name.isdigit():
                continue
            if episode_directory.name in known_ids:
                continue
            manifest_path = episode_directory / "episode.yaml"
            if not manifest_path.exists():
                continue
            with manifest_path.open(encoding="utf-8") as manifest_file:
                manifest = yaml.safe_load(manifest_file) or {}
            manifest["id"] = episode_directory.name
            manifest["recovered_after_restart"] = True
            self._episode_results.append(manifest)
        self._episode_results.sort(key=lambda item: int(item["id"]))
...
def _send_status(node, state, **details):
    """Send a JSON recorder acknowledgement to the collection UI."""
    payload = {
        "state": state,
    ...
    node.send_output(
        "status",
        pa.array([json.dumps(payload, sort_keys=True)], type=pa.string()),
    )
...
def main():
    ...
    _send_status(
        node,
        "ready",
        dataset=str(dataset_writer._base_directory),
        completed_episodes=len(dataset_writer._episode_results),
    )
    ...
    for event in node:
        ...
        if event_id == "command":
            ...
            if command == "start":
                if episode is not None:
                    _send_status(node, "error", message="episode already recording")
                    continue
                episode = Episode()
                episode.number = event["metadata"].get("episode_number", 0)
                episode.task_index = event["metadata"].get("task_index", 0)
                episode.started_at_ns = time.time_ns()
                session_json = event["metadata"].get("session_json", "{}")
                try:
                    episode.session = json.loads(session_json)
                ...
                try:
                    ...
                    episode_writer = None
                    continue
                _send_status(node, "recording", episode_number=episode.number)
            elif command in ("success", "fail"):
                if episode is None or episode_writer is None:
                    _send_status(node, "error", message="no active episode")
                    continue
                if command == "success":
                    episode.success = True
                else:
                    episode.failure_reason = event["metadata"].get(
                        "failure_reason", "unclassified"
                    )
                episode.ended_at_ns = time.time_ns()
                episode.stream_counts = {
                    stream: int(count)
                    for stream, count in episode.stream_counts.items()
                }
                missing_streams = sorted(
                    stream
                    for stream in REQUIRED_EPISODE_STREAMS
                    if episode.stream_counts.get(stream, 0) == 0
                )
                if missing_streams:
                    episode.success = False
                    episode.failure_reason = "data_health"
                if episode.qc_errors:
                    episode.success = False
                    episode.failure_reason = "data_health"
                episode.status = "accepted" if episode.success else "failed"
                try:
                ...
                if episode is None or episode_writer is None:
                    _send_status(node, "error", message="no active episode")
                    continue
                destination = episode_writer.cancel()
                _send_status(
                    node,
                    "cancelled",
                    episode_number=episode.number,
                    quarantine=str(destination) if destination else None,
                )
                episode = None
                episode_writer = None
            elif command == "quit":
                if episode is not None:
                    ...
                    _send_status(
                        ...
                        "cancelled",
                        episode_number=episode.number,
                        reason="quit_during_episode",
                        quarantine=str(destination) if destination else None,
                    )
                    episode = None
                    episode_writer = None
                _send_status(node, "stopped")
                break
            continue
        ...
        if episode is None:
            continue
        episode.stream_counts[event_id] = episode.stream_counts.get(event_id, 0) + 1
        timestamp = event["metadata"].get(
            "capture_timestamp_ns", event["metadata"]["timestamp"]
        )
        ...
        previous_timestamp = episode.last_timestamps.get(event_id)
        if previous_timestamp is not None and timestamp <= previous_timestamp:
            episode.qc_errors.append(
                {
                    "stream": event_id,
        ...
        episode.last_timestamps[event_id] = timestamp
