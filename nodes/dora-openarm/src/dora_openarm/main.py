...
def _align(arm, state, new_position, name, threshold, trigger=None):
    ...
    if trigger == "gripper":  # Check if gripper is active (threshold ~ -10 deg)
        ...
        if name == "right_arm":
            ...
            is_gripping = gripper_position < np.deg2rad(5)
        if not is_gripping:
            # Keep the seven arm joints stationary while alignment is gated,
            # but continue accepting the gripper target. In particular, this
            # lets a released trigger reopen the gripper even if this arm has
            # not completed alignment yet.
            gripper_only_position = current_position.copy()
            gripper_only_position[-1] = gripper_position
            arm.send_position(gripper_only_position)
            return False
