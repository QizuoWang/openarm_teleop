...
_DEFAULT_GRIPPER_ENDPOINTS = {
    "right": {"open": -1.0, "closed": 0.0},
    "left": {"open": 1.0, "closed": 0.0},
}


def _load_gripper_endpoints(path: str | None) -> dict[str, dict[str, float]]:
    endpoints = {
        side: values.copy() for side, values in _DEFAULT_GRIPPER_ENDPOINTS.items()
    }
    if not path:
        return endpoints

    calibration_path = pathlib.Path(path).expanduser().resolve()
    with calibration_path.open("r", encoding="utf-8") as stream:
        document = json.load(stream)
    for side in ("right", "left"):
        values = document.get(side)
        if not isinstance(values, dict):
            raise ValueError(f"missing gripper calibration for {side}: {calibration_path}")
        endpoints[side] = {
            "open": float(values["open"]),
            "closed": float(values["closed"]),
        }
    return endpoints
...
def _map_trigger_to_gripper(
    trigger: float,
    side: str,
    endpoints: dict[str, dict[str, float]] | None = None,
) -> float:
    """Linearly map a released/pressed trigger to calibrated motor endpoints."""
    trigger = float(np.clip(trigger, 0.0, 1.0))
    values = (endpoints or _DEFAULT_GRIPPER_ENDPOINTS)[side]
    return values["open"] + (values["closed"] - values["open"]) * trigger
...
def _run(args: argparse.Namespace) -> None:
    ...
    gripper_endpoints = _load_gripper_endpoints(args.gripper_calibration)
    print(f"[receiver] gripper calibration: {gripper_endpoints}")
    ...
    for event in node:
        ...
        if pose_right is not None and "rt" in msg:
            gripper_angle = _map_trigger_to_gripper(
                float(msg["rt"]), "right", gripper_endpoints
            )
        ...
        if pose_left is not None and "lt" in msg:
            gripper_angle = _map_trigger_to_gripper(
                float(msg["lt"]), "left", gripper_endpoints
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--gripper-calibration",
        default=os.getenv("GRIPPER_CALIBRATION_FILE"),
        help="JSON file containing measured right/left open and closed endpoints.",
    )
